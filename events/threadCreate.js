...
module.exports = {
    ...
    async execute(thread) {
        ...
        console.log(`[${new Date().toISOString().replace('T', ' ').substring(0, 19)}] 🧵 Thread created - #${thread.name} in #${thread.parent ? thread.parent.name : 'Unknown'} - Server: ${thread.guild.name}`);

        const embed = new EmbedBuilder()
            .setTitle("🧵 Thread created")
            .setDescription(`**Name:** ${thread}\n**Parent channel:** ${thread.parent ? thread.parent : 'Unknown'}\n**Type:** ${thread.type}`)
            .setColor('#2ECC71')
            .setTimestamp();
